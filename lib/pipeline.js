...
export async function run({ position } = {}) {
  const t0 = Date.now();
  ...
  // 叙事 + 持仓
  report.narrative = await narrative(report, position);
  report.position = report.narrative && report.position ? report.position : null;
